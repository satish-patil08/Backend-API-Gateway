    ...
    stages {
        ...
        stage('Delete the old containers and deploy new JAR') {
            steps {
                ...
                sudo systemctl stop backend-api-gateway || echo "Service already stopped or does not exist"

                # Clean up the deployment directory
                sudo rm -rf /var/lib/jenkins/backend/backend-api-gateway/*

                # Copy the new JAR file to the deployment directory
                sudo cp -r target/*.jar /var/lib/jenkins/backend/backend-api-gateway/

                # Change ownership of the files to the appropriate user
                sudo chown -R azureuser:azureuser /var/lib/jenkins/backend/backend-api-gateway/

                # Rename the JAR file to app.jar for consistency
                sudo mv /var/lib/jenkins/backend/backend-api-gateway/*.jar /var/lib/jenkins/backend/backend-api-gateway/app.jar
                '''
            }
        }
        ...
        stage('Run the updated service') {
            steps {
                sh '''
                sudo systemctl start backend-api-gateway

                # Check the status of the service to ensure it started successfully
                sudo systemctl status backend-api-gateway --no-pager
                '''
            }
        }
    }
